...
public abstract class Refactoring extends TwoPassTransformation {
	...
	//统计所选类中用于重构的可用元素的数量。如果某个元素不适用于当前重构，则不计算该元素
	...
	//在类中查找元素与该类型的其他元素相关的位置，而不是它们是否可以重构。
	...
	//使用其名称查找编译单元中的相关元素，如果可以使用当前重构对其进行重构，则返回其位置。
	...
	// Returns all the "super." references in a member declaration.�������С�super����Ա�����е����á�
...
//		int firstDash = (unit.indexOf("MultiRefactor\\") == -1) ? 5 : unit.indexOf("MultiRefactor\\") + 14;
		int firstDash = (unit.indexOf("MyRef\\") == -1) ? 5 : unit.indexOf("MyRef\\") + 14;
